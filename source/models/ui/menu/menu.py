...
class CommandMenu:
    def __init__(self) -> None:
        ...
        self.action_commands = {
            'attack': {'label':'ATTACK', 'description': 'Launches a basic attack at an enemy.','is_selected' : False, 'is_hovered' : False},
            'skills' : {'label':'SKILLS', 'description' : 'Special skills unique to each character.', 'is_selected' : False, 'is_hovered' : False}, 
            'defend': {'label': 'DEFEND','description': 'Enter a defensive stance reducing damage taken by 50\% until next turn.', 'is_selected' : False, 'is_hovered' : False}, 
            'item' : {'label': 'ITEM','description': 'Use an item in stock.', 'is_selected' : False,'is_hovered' : False}, 
            'limit_break' : {'label' : 'LIMIT BREAK', 'description': 'Uses limit break against an enemy.', 'is_selected' : False, 'is_hovered' : False}
            }
    ...
    def draw_menu_item(self, command: dict, mouse_pos: tuple, mouse_pressed: bool, x_pos: int, y_pos: int, text_color = MENU_TEXT_COLOR, disabled: bool = False):
        ...
        if self.action_commands[command]['is_selected']:             
            selected_option_rect.topleft = ((command_text_rect.topleft[0] -20,command_text_rect.topleft[1] -5) )
